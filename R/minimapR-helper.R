...
minimap2_installation <- function(source_directory, verbose = TRUE, return = FALSE) {
  ...
  if (.Platform$OS.type == "windows") {
    if (!minimap2_path) {
      ...
      cat("3. In the 'MSYS2' terminal, type 'pacman -S mingw-w64-x86_64-samtools autotools gcc git'\n")
      cat("4. add C:\\msys64\\mingw64\\bin to windows PATH'\n")
      ...
      cat("6. Create symbolic link to 'minimap2', 'MSYS2' command: 'ln -s ~/minimap2/minimap2 C:/mingw64/bin'\n")
    } else {
      cat("'minimap2' is already installed. \n")
    }
